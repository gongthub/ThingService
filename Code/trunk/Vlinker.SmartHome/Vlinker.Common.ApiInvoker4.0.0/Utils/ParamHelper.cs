...
namespace Vlinker.Common.ApiInvoker
{
    public class ParamHelper
    {
        ...
        public const string COOKIE_PREFIX = "SmartHome_";
        ...
        public static Task<SsoUserInfo> GetUserInfo()
        {
            ...
            {
                return Task.Factory.StartNew<SsoUserInfo>(() =>
                {
                    return null;
                }); 
            }
            ...
            try
            {
                ...
                return Task.Factory.StartNew<SsoUserInfo>(() =>
                {
                    return JsonConvert.DeserializeObject<SsoUserInfo>(userInfo);
                }); 
            }
            catch
            {
        ...
        {
            HttpCookie uuid = HttpContext.Current.Response.Cookies[COOKIE_PREFIX + uuidkey];

            string strValue = (uuid == null ? "" : (uuid.Value == null ? "" : uuid.Value));

            if (uuid == null || strValue == "")
            {
                uuid = HttpContext.Current.Request.Cookies[COOKIE_PREFIX + uuidkey];
            }
        ...
        {
            HttpCookie uuid = HttpContext.Current.Response.Cookies[COOKIE_PREFIX + hashkey];

            string strValue = (uuid == null ? "" : (uuid.Value == null ? "" : uuid.Value));

            if (uuid == null || strValue == "")
            {
                uuid = HttpContext.Current.Request.Cookies[COOKIE_PREFIX + hashkey];
            }
        ...
        private bool SetCookies(string hashkey, string uuidkey, string value)
        {
            ...
            ctx.Response.Cookies.Remove(COOKIE_PREFIX + uuidkey);
            ...
            ctx.Response.Cookies.Remove(COOKIE_PREFIX + hashkey);
